...
def linear_combination(i1: np.ndarray, i2: np.ndarray, a1: float, a2: float) -> np.ndarray: 
    ...
    # Salvar a imagem resultante com o mesmo formato das imagens
    # (...)
    ext = "jpg"
    #
    output_path = f'result.{ext}'
    cv2.imwrite(output_path, i_out)

    # Exibir a imagem resultante
    cv2.imshow('Linear Combination', i_out)
    cv2.waitKey(0) 
    cv2.destroyAllWindows()
    print("Imagem exibida e programa finalizado.")
